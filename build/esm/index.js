...
function generalMic(e, n) {
    ...
    for (let e = 0; e < f; e++) {
        var [, t] = o[e];
        const c = binShareRange(t, l, a);
        var r = entropy(rangeNormilize(c.filter(e => 0 < e))),
            t = t.length / n.length;
        i += t * r, vecAdd(s, c)
    }
...
function incSim(t, r, n, i, e) {
    ...
    for ([l, a] of g.entries()) {
        var u, p = a.length / e;
        0 != p && (!(a.length < BIN_SIZE ** 2) && h.has(l) ? (u = h.get(l), c += p * l2Dis2(normalizeScatter(a), normalizeScatter(u))) : c += p)
    }
...
class NextVICore {
    ...
    recommandFilter(r) {
        ...
        if (0 < (a = void 0 !== r.filters ? e.filter(t => !r.filters.find(e => e.fid === t.fid)) : [...e]).length) {
            let i = "",
                ...
                n = 0;
            for (let t of a) {
                const p = this.dataSource.map(e => e[t.fid]);
                let r = 0;
                l.forEach(t => {
                    var e = this.dataSource.map(e => e[t.fid]),
                        e = generalMic(p, e);
                    r += e
                }), (r /= l.length) > n && (n = r, i = t.fid, e = p)
            }
            if ("" !== i) {
                const t = new Set(e);
                    ...
                    if (0 !== d.length) {
                        ...
                        for (let r of l) {
                            var s = this.dataSource.map(e => e[r.fid]),
                                ...
                                c = this.dataSource.filter(e => e[i] === n).map(e => e[r.fid]),
                                u = rangeNormilize(binMapShareRange(c, g[0], g[1]));
                            let t = 0;
                            for (let e = 0; e < h.length; e++) 0 < h[e] && 0 < u[e] && (t += h[e] * Math.log2(h[e] / u[e]));
                            e += t * (c.length / s.length)
                        }
                        e /= l.length, t.push({
                            fid: d[0].fid,
                            type: "set",
    ...
    fewatureSelectionForSecondPatternWithSpecifiedViews(e, t) {
        ...
        for (let t = 0; t < o.length; t++) {
            var c = incSim(l.map(e => e[o[t].fid]), f, a.map(e => e[o[t].fid]), s, r.length);
            c > g && (g = c, h = t)
        }
        return -1 < h ? {
            features: [o[h]],
...
function autoSet(e, n, t, r) {
    ...
    var h, c, u = getFieldRelationMatrix(e, n);
    const p = new Array(n.length).fill(!1).map(() => new Array(n.length).fill(!1));
    for (h of t) {
        var d = h.fields.filter(e => "*" !== e).map(e => a.get(e.fid));
        for (let t = 0; t < d.length; t++)
            for (let e = 0; e < d.length; e++) d[t] !== d[e] && (p[d[t]][d[e]] = p[d[e]][d[t]] = !0)
    }
    const m = [];
    for (c of t) {
        const v = c.fields;
        let r = c.fields.filter(e => "*" !== e);
        if (!c.locked && v.length > r.length) {
            var I = v.filter(e => "*" === e);
            if (I.length === v.length) {
                ...
                r.push(..._.map(e => n[e])), markUsedRelation(p, B);
                for (let e = 0; e < p.length; e++) p[e][e] = !1
            } else {
                g.filter(t => v.find(e => "*" !== e && e.fid === t[0]));
                let e = I.length;
                for (let e = 0; e < p.length; e++) p[e][e] = !0;
                const E = v.filter(e => "*" !== e),
                    R = i.map(e => a.get(e.fid));
                var M = getSubMatrix(u, R);
                for (let t = 0; t < E.length; t++) {
                    const w = getSubMatrix(p, R);
                    var y = i.findIndex(e => e.fid === E[t].fid);
                    const T = recInternalRelation(M, w, y);
                    null === T || r.find(e => e.fid === n[R[T.to]].fid) || (w[T.from][T.to] = w[T.to][T.from] = !0, p[R[T.from]][R[T.to]] = p[R[T.to]][R[T.from]] = !0, e--, r.push(n[R[T.to]]))
                }
                const {
                    vertexIndices: Z,
                    leftWildCardNum: A,
                ...
                e = A, r.push(...Z.map(e => n[e])), markUsedRelation(p, F);
                for (let e = 0; e < p.length; e++) p[e][e] = !1
            }
        } else r = c.fields.filter(e => "*" !== e);
        m.push({
            ...c,
...
function nnMic(r, n, e = 0) {
    ...
    for (f = 0; f < o; f++) {
        ...
        const p = binGroupByShareFreqRange(a[f][1], i);
        var c = entropy(rangeNormilize(p.filter(e => 0 < e)));
        s += c * h, vecAdd(g, p)
    }
    if (a.length > o) {
        let e = new Array(BIN_SIZE).fill(0),
            t = 0;
        for (; f < a.length; f++) {
            var u = binGroupByShareFreqRange(a[f][1], i);
            t += a[f][1].length, vecAdd(e, u)
        }
        t /= n.length;
...
function purennMic(e, t, r = 0) {
    ...
    for (f = 0; f < o; f++) {
        ...
        var c = entropy(rangeNormilize(u.filter(e => 0 < e)));
        s += c * h, vecAdd(g, u)
    }
